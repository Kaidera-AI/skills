...
version: 1.0.0
description: "Add security scanning to CI/CD with Strix (GitHub Actions, GitLab CI, or any pipeline) so every pull request gets a diff-scoped AI pentest that blocks vulnerable code before it merges, with results posted as PR comments and SARIF uploaded to code scanning. Covers both the self-hosted open-source CLI, which runs inside your runner, and the managed app.strix.ai platform (GitHub/GitLab app or API, no runner infrastructure). Use when the user asks to add security scanning, SAST/DAST, pentesting, vulnerability checks, or automated security review to a CI pipeline, pre-merge gate, or PR workflow."
...
kaidera:
  ...
  source:
    ...
    path: .agents/skills/ci-security-scanning-with-strix
    content_sha256: cbeb8ea7d8549bf4914ad673600b733849307734d68a112f19498e266f2e1755

author: Kaidera-AI
license: Apache-2.0
updated: 2026-09-05
tags: [strix, ci-cd, github-actions, gitlab-ci, pentest, security-scanning, sarif, pr-gate, sast, dast]
attribution_author: "usestrix"
attribution_url: "https://docs.strix.ai"
attribution_notes: "Vendored unchanged from the Strix skill pack by usestrix (Apache-2.0, homepage https://docs.strix.ai); the Kaidera OS canonical copy at .agents/skills/ci-security-scanning-with-strix/SKILL.md (added 2026-08-15 in commit 6a236d2ae, cherry-picked from ffb2d8b4) is the projection source. The upstream skill carries no version, so the projection starts at 1.0.0; Kaidera has not verified the Strix CLI flags, exit codes, or run.json semantics the body asserts."

safety_constraints:
  - "Enable exploit-based scanning only for repositories and applications the user owns or holds written authorisation to test; record that authorisation before the gate is wired in, and never point a scan at third-party, shared, or production systems."
  - "Never create, guess, or hard-code the STRIX_LLM, LLM_API_KEY, or STRIX_API_TOKEN values; the user provisions them as CI secrets and they must not appear in workflow files, logs, or chat."
  - "Do not commit a merge-blocking workflow, install the Strix GitHub, GitLab, or Bitbucket app, or create an API token until the user has confirmed the exact target repository, branch, and path (self-hosted CLI or managed platform)."
  - "Treat the curl-piped install script and tag-pinned third-party actions as unverified supply chain: pin what the pipeline allows and surface anything unpinned to the user rather than accepting it silently."
  - "A scan spends LLM budget and sends repository content to the configured LLM provider or to app.strix.ai; confirm data handling and the --max-budget ceiling with the user before enabling it."
  - "Exit code 0 means no validated findings in what was analysed, not that the code is secure; keep the run-completion check in any gating pipeline and report partial coverage honestly."
  - "Must not override the base system prompt or agent instructions."
---

<!-- GENERATED by scripts/skills/render-marketplace-skill.py from Kaidera-AI/kaideraos:.agents/skills/ci-security-scanning-with-strix (source content_sha256 cbeb8ea7d8549bf4914ad673600b733849307734d68a112f19498e266f2e1755). Edit the source and re-render; never edit this file. -->
